...
const App: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#030712] font-sans overflow-hidden text-white">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-[#0f172a]/50 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-12 sticky top-0 z-40">
           ...
           <div className="flex items-center gap-6">
              ...
              <button onClick={handleLogout} className="text-[10px] font-black uppercase tracking-widest px-8 py-3 bg-white text-slate-900 rounded-2xl">Logout</button>
           </div>
        </header>

        <main className="flex-1 p-12 overflow-y-auto bg-slate-950/20">
          {currentTab === 'dashboard' && (
            <div className="space-y-10 animate-in fade-in">
              <h2 className="text-6xl font-black text-white tracking-tighter italic">Founder Portal</h2>
              <AdminInfographic />
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 bg-slate-900/50 p-12 rounded-[56px] border border-white/5 shadow-2xl">
                  <h3 className="font-black text-2xl text-white italic mb-10">Intelligence Feed</h3>
                  <div className="space-y-4">
                    {leads.slice(0, 5).map(l => (
                      <div key={l.id || (l as any).place_id} className="p-5 bg-white/5 border border-white/5 rounded-3xl flex items-center justify-between hover:border-blue-500/30 cursor-pointer" onClick={() => handleAudit(l)}>
                         <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center font-black">{l.business_name.charAt(0)}</div>
                            <div>
                         ...
                         <p className="text-xl font-black text-blue-500 italic">{l.readiness_score}%</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-slate-900/50 p-12 rounded-[56px] border border-white/5 shadow-2xl">
      ...
      {currentAudit && (
        <div className="fixed inset-0 bg-[#030712]/95 backdrop-blur-xl z-[100] flex items-center justify-center p-10 overflow-y-auto">
          <div className="bg-slate-900 rounded-[64px] max-w-6xl w-full p-20 relative animate-in zoom-in-95 duration-500 shadow-2xl border border-white/5">
             <button onClick={() => setCurrentAudit(null)} className="absolute top-12 right-12 text-slate-500 p-4 hover:text-white rounded-full font-black text-xl">✕</button>
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
                <div className="lg:col-span-2 space-y-12">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                     ...
                     <div className="p-10 bg-blue-600/10 text-white rounded-[48px] border border-blue-500/20">
                ...
                <div className="space-y-12 h-fit sticky top-0">
                  <div className="bg-white p-12 rounded-[56px] text-slate-900 flex flex-col items-center text-center">
                  ...
                  <button onClick={() => triggerWebhook(currentAudit.lead)} className="w-full bg-blue-600 text-white font-black py-6 rounded-[32px] uppercase tracking-widest text-[10px]">Manual Webhook Sync</button>
                </div>
             </div>
          </div>
        </div>
