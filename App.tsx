...
const App: React.FC = () => {
  ...
  const triggerWebhook = async (data: any) => {
    setSignals(prev => [{ id: Math.random().toString(), text: `InsForge Signal: ${data.business_name || 'Generic'}`, type: 'webhook', time: new Date().toLocaleTimeString() }, ...prev].slice(0, 15));
    try {
    ...
    try {
      ...
      if (toolCalls?.length) {
        for (const call of toolCalls) {
          if (call.name === 'trigger_n8n_signal') await triggerWebhook({ ...call.args, lead_id: lead.id });
        }
      }
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans selection:bg-blue-100">
      ...
      {isAuditing && <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex flex-col items-center justify-center text-white"><div className="w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-4 shadow-[0_0_50px_rgba(37,99,235,0.4)]"></div><p className="font-black uppercase tracking-[0.4em] text-[10px]">Syncing Neural Path...</p></div>}
