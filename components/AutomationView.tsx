    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-700 mt-1 font-medium italic">Gemini Tool-Dispatch linked to n8n node infrastructure.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-[#0f172a] rounded-[48px] p-12 text-white shadow-2xl border border-white/5 space-y-8">
           ...
           <p className="text-slate-400 font-medium">This MCP-compliant URI receives autonomous tool calls from the Gemini AI Orchestrator.</p>
           <div className="bg-white/5 border border-white/10 p-6 rounded-2xl font-mono text-[11px] break-all text-blue-300">
             {webhookUrl}
           </div>
        ...
        <div className="bg-white rounded-[48px] border border-slate-200 p-12 shadow-sm space-y-6">
           <h3 className="text-2xl font-black text-slate-900 tracking-tighter">Live Status Logs</h3>
           <div className="space-y-4">
             {[
               { icon: '📡', text: 'n8n Webhook Listener Initialized', time: 'SYSTEM' },
               { icon: '🧠', text: 'Gemini 3.0 Pro Tool definitions synchronized', time: 'SYSTEM' },
               { icon: '✓', text: 'Autonomous dispatch gate open', time: 'READY' }
             ].map((log, i) => (
        ...
        {workflows.map((wf) => (
          <div key={wf.id} className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm space-y-6 group hover:border-blue-500 transition-all">
             <div className="flex justify-between items-start">
               ...
               <div className={`w-2 h-2 rounded-full ${wf.status === 'active' ? 'bg-green-500' : 'bg-slate-300'}`}></div>
             </div>
             <div>
               <h4 className="font-bold text-slate-900 leading-tight">{wf.name}</h4>
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Success: {wf.successRate}%</p>
             </div>
          </div>
        ))}
      </div>
