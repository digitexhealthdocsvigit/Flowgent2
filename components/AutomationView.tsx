  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {workflows.map((wf) => (
          <div key={wf.id} className={`bg-white p-8 rounded-[32px] border transition-all ${wf.status === 'active' ? 'border-slate-200 shadow-sm' : 'border-slate-100 opacity-60 bg-slate-50'}`}>
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${wf.status === 'active' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-700 font-black uppercase tracking-widest mt-1">Last Run: {wf.status === 'active' ? wf.lastRun : 'Offline'}</p>
                </div>
              </div>
              <button 
                onClick={() => onToggleStatus(wf.id)}
                className={`w-12 h-6 rounded-full relative transition-colors ${wf.status === 'active' ? 'bg-green-500' : 'bg-slate-300'}`}
              >
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${wf.status === 'active' ? 'left-7' : 'left-1'}`}></div>
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 mt-8">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <p className="text-xl font-black text-slate-900">{wf.status === 'active' ? wf.successRate : 0}%</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1">Triggers Today</p>
                <p className="text-xl font-black text-slate-900">{wf.status === 'active' ? '142' : '0'}</p>
              </div>
            </div>

            <div className="mt-8 flex gap-3">
              <button className="flex-1 py-3 bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-all active:scale-95">Configure</button>
              <button className="px-4 py-3 bg-slate-100 text-slate-700 font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-slate-200 transition-all border border-slate-200 active:scale-95">Logs</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-900 p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
        ...
        <button onClick={() => window.open('https://github.com/digitexhealthdocsvigit/Flowgent', '_blank')} className="mt-8 px-8 py-4 bg-white text-slate-900 font-black text-xs uppercase tracking-[0.2em] rounded-2xl hover:bg-slate-100 transition-all active:scale-95">View GitHub Actions</button>
      </div>
    </div>
  );
};
