...
const SettingsView: React.FC<SettingsViewProps> = ({ webhookUrl, onUpdate, onTest, activeProjectRef }) => {
  ...
  const handleSave = () => {
    ...
    alert("Neural Bridge Config Persistent.");
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center">
        ...
        <div className="bg-slate-900 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest italic border border-white/5">Build: v2.9.0-foundation</div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-8">
          ...
          <div className="bg-[#0f172a] p-12 rounded-[56px] text-white border border-white/5 shadow-2xl flex flex-col justify-between relative overflow-hidden group">
             ...
             <button onClick={checkHealth} className="mt-8 text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-white transition-colors text-left underline italic">Re-verify Infrastructure</button>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 p-12 rounded-[56px] border border-white/5 shadow-2xl space-y-8 backdrop-blur-xl">
           <div className="flex justify-between items-center">
             <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-black text-white italic">InsForge MCP Hub</h3>
             </div>
             <button 
             ...
             >
               {showMcpGuide ? 'Hide Setup' : 'Show Setup'}
             </button>
           </div>

           <div className="space-y-6">
             <p className="text-slate-400 text-sm font-medium leading-relaxed italic">
               Connect Flowgent directly to your local AI assistant (Claude Desktop) using the Model Context Protocol.
             </p>
             
             {showMcpGuide ? (
               <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                 <div className="bg-black/40 rounded-3xl p-8 space-y-4 border border-white/5">
                    <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Claude Desktop Config Payload</p>
                    <pre className="text-[10px] font-mono text-blue-300 overflow-x-auto p-4 bg-black/20 rounded-xl custom-scrollbar">
                      {mcpConfig}
                    </pre>
                    <button 
                      onClick={() => { navigator.clipboard.writeText(mcpConfig); alert("MCP Config Copied."); }}
                      className="w-full bg-white/5 hover:bg-white/10 text-white font-black py-4 rounded-2xl text-[9px] uppercase tracking-widest transition-all"
                    >
                      Copy Payload
                    </button>
                 </div>
                 <div className="p-6 bg-blue-600/10 rounded-3xl border border-blue-500/20">
                    <h4 className="text-[10px] font-black text-blue-400 uppercase mb-2">Activation Steps:</h4>
                    <ul className="text-[10px] text-slate-300 space-y-2 list-disc pl-4 font-bold">
                       <li>Locate claude_desktop_config.json on your machine.</li>
                       <li>Paste the payload into the "mcpServers" block.</li>
                       <li>Restart Claude and prompt: "Research my InsForge leads."</li>
                    </ul>
                 </div>
               </div>
             ) : (
               <div className="grid grid-cols-1 gap-4">
                  {[
                    { label: 'Agentic Connectivity', val: 'Active', icon: '🤖' },
                    { label: 'Documentation Fetch', val: 'Enabled', icon: '📄' },
                    { label: 'Schema Validation', val: 'Strict', icon: '🛡️' }
                  ].map((feat, i) => (
                    <div key={i} className="bg-white/5 border border-white/5 p-6 rounded-[28px] flex justify-between items-center group hover:border-purple-500/30 transition-all">
                       <div className="flex items-center gap-4">
                          <span className="text-xl">{feat.icon}</span>
                          <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">{feat.label}</span>
                       </div>
                       <span className="text-[10px] font-black text-purple-400">{feat.val}</span>
                    </div>
                  ))}
               </div>
             )}
